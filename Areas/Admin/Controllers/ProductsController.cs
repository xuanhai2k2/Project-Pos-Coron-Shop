...
namespace devpro_project.Areas.Admin.Controllers
{
    ...
    public class ProductsController : Controller
    {
        ...
        public IActionResult Read(int? page)
        {
            ...
            return View("Read", products.ToPagedList(pageNumber, pageSize));
        }
        //update
        ...
        public IActionResult UpdatePost(int id, IFormCollection fc)
        {
            ...
            double _Price = Convert.ToInt32(fc["Price"].ToString());
            double _Discount = Convert.ToInt32(fc["Discount"].ToString());
            int _Hot = !String.IsNullOrEmpty(fc["Hot"]) ? 1 : 0;
            string _Description = fc["Description"].ToString();
            string _Content = fc["Content"].ToString();
            //lay record tuong ung voi id truyen vao
            ItemProduct record = db.Products.FirstOrDefault(item=>item.Id == id);
            if (record != null)
            {
                record.Name = _Name;
            ...
            //Xoas các bản ghi tương ứng với ProductId trong bản CategoriesProducts để update lại bản ghi mới
